...
for k = 1:q
    %randID = floor(rand(1)*n_train)+1; % Get a random index from the training dataset  
...
%     filter2(filter2 == Inf) = 1;
    ...
    %%K_user = K_user .* exp(-(AgeGroup(user_r) - AgeGroup).^2);
    %%K_user = (K_user).*(K_user >= 0.001);
...
%%%     d_movie_filter3 = real(acos( (genres*genres(mov_r,:)') ./ (normGenres*normGenres(mov_r)) ));
%%%     movie_filter3 = (1 - d_movie_filter3.^2).*(d_movie_filter3 < h2);
%%%     movie_filter3(movie_filter3 == 0) = 0.1;
    %%d_movie_filter2 = 1 - (2/pi)*real(acos( (genres*genres(mov_r,:)') ./ (normGenres*normGenres(mov_r)) ));
    %%movie_filter2 = max((3/4)*(1 - (d_movie_filter2/h2).^2),0);
    %movie_filter = double(full(M(user_r,:) ~=0));
    %movie_filter(movie_filter == 0) = 0.85;
    %K_mov = K_mov .* full(movie_filter)';
    
    %%K_mov = K_mov .* full(movie_filter2);
    %%%K_mov = K_mov .* full(movie_filter3);
